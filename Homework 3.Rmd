...
#libra
set.seed(0)
split = sample(nrow(BreastCancer), size=round(nrow(airquality) * .75))
train_regression = na.omit(BreastCancer[-split,])
test_regression = na.omit(BreastCancer[split,])

train_log <- train_regression[c(which(train_regression$Class== "benign"), which(train_regression$Class == "malignant")),]
test_log <- test_regression[c(which(test_regression$Class == "benign"), which(test_regression$Class == "malignant")),]
train_log$Class <- factor(train_regression$Class)
test_log$Class <- factor(test_regression$Class)
ctrl <- trainControl(method = "repeatedcv", repeats = 20 ,classProbs = T,savePredictions = T)

log_regression <- train(Class~ Cl.thickness + Cell.size + Cell.shape + Marg.adhesion + Epith.c.size + Bare.nuclei + Bl.cromatin + Normal.nucleoli + Mitoses, data=train_regression, method = "LogitBoost", family= "binomial", trControl = ctrl)

```

I used boosted logistic regression because it is a parametric model which fits the problem at hand: predicting one of two categories given numerical parameters from data. It requires little hyperparameter tuning, it is computationally efficient, and it is evaluated well by repeated cross validation. 
...
plot(x = roc(predictor = log_regression$pred$benign,
             response = log_regression$pred$obs)$specificities, 
     y = roc(predictor = log_regression$pred$benign, 
             response = log_regression$pred$obs)$sensitivities,
     col= "blue", xlim = c(1, 0), type ="l", ylab = "Sensitivity",
     xlab = "Specificity")
legend("bottomright", legend = paste("Benign vs Malignant -- ", 
                                     roc(predictor = log_regression$pred$benign,response = log_regression$pred$obs)$auc, sep = ""), col = c("blue"), fill = c("blue"))
...
```{r}
predictions <- predict(log_regression, newdata = test_log)
confusionMatrix(predictions, 
                reference = test_log$Class)
```
